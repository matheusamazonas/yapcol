...
/// Applies `parser` between 0 and a given number of times, ensuring that no more matches, without
/// collecting possible matches.
///
/// # Outcome
///
/// This combinator succeeds if the argument parser succeeds between 0 and up to (and including)
/// `max_count` times. Unlike [`crate::many0_up_to`], this combinator doesn't return its matches,
...
/// # Shortcut
///
/// This combinator has a shortcut version: [`Parser::many0_up_to_discard`].
///
/// # Arguments
///
...
/// let max_count = 2;
/// assert!(many0_up_to_discard(&parser, max_count)(&mut input).is_err());
///
/// // Succeeds on empty input if `max_count` is 0.
/// let mut input = Input::new_from_chars("".chars(), None);
/// let max_count = 0;
/// assert_eq!(many0_up_to_discard(&parser, max_count)(&mut input), Ok(0));
/// ```
pub fn many0_up_to_discard<P, IT, O>(parser: &P, max_count: usize) -> impl Parser<IT, usize>
where
	P: Parser<IT, O>,
	IT: InputToken,
{
	move |input| match many_no_end(parser, 0, Some(max_count), false)(input) {
...
mod tests {
	...
	fn empty() {
		let parser = is('h');
		let mut input = Input::new_from_chars("".chars(), None);
		let parser_up_to = many0_up_to_discard(&parser, 1);
	...
	#[test]
	fn empty_shortcut() {
		let parser = is('h').many0_up_to_discard(1);
		let mut input = Input::new_from_chars("".chars(), None);
	...
	fn less_than_max_count() {
		...
		let parser_up_to = many0_up_to_discard(&parser, 3);
		let output = parser_up_to(&mut input).unwrap();
		assert_eq!(output, 2);
	...
	fn zero_empty() {
		let parser = is('h');
		let mut input = Input::new_from_chars("".chars(), None);
		let parser_up_to = many0_up_to_discard(&parser, 0);
		let output = parser_up_to(&mut input).unwrap();
		assert_eq!(output, 0);
