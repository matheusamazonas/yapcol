...
/// `max_count` times. Unlike [`crate::once_up_to_collect`], this combinator doesn't return the
...
/// # Shortcut
///
/// This combinator has a shortcut version: [`Parser::once_up_to`].
///
/// # Arguments
///
...
/// # Panics
///
/// This function panics if `max_count` is equal to 0. Check [`crate::up_to`] if you
/// would like to cover this case.
///
/// # Examples
///
/// ```
/// use yapcol::{Input, is, once_up_to};
///
/// // Succeeds if the parser matches exactly `max_count` times.
/// let parser = is('1');
/// let mut input = Input::new_from_chars("112".chars(), None);
/// let max_count = 2;
/// assert_eq!(once_up_to(&parser, max_count)(&mut input), Ok(2));
///
...
/// let max_count = 2;
/// assert!(once_up_to(&parser, max_count)(&mut input).is_err());
/// ```
pub fn once_up_to<P, IT, O>(parser: &P, max_count: usize) -> impl Parser<IT, usize>
where
	P: Parser<IT, O>,
	IT: InputToken,
{
	...
	move |input| match repeat_no_end(parser, 1, Some(max_count), false)(input) {
...
mod tests {
	...
	fn empty() {
		...
		let parser_up_to = once_up_to(&parser, 1);
		assert_eq!(
	...
	fn less_than_max_count() {
		...
		let parser_up_to = once_up_to(&parser, 3);
		let output = parser_up_to(&mut input).unwrap();
		assert_eq!(output, 2);
